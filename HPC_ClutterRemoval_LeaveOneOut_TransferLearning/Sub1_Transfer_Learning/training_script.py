...
select_sample_num = 40*30
transfer_data_ratio = select_sample_num / loo_data_num
# TODO: transfer learning data without noise sample
# remove noise samples from the dataset, only  6000 samples left
...
for train_ix, test_ix in rskf.split(X=X_mmw_rD_transfer, y=np.argmax(Y_transfer, axis=1)):
    ...
    for feed_in_ratio in feed_in_ratios:
        ...
        transfer_model_csv_log_path = os.path.join(transfer_info_dir, str(split_round) + '_' + str(feed_in_ratio) + "_model_history_log.csv")
        csv_logger = CSVLogger(filename=transfer_model_csv_log_path,
                               append=True)

        # transfer best model bath
        best_transfer_model_path = os.path.join(transfer_info_dir, str(split_round) + '_' + str(feed_in_ratio)+ '_best_transfer_model.h5')
        mc = ModelCheckpoint(
