...
load_data_dir = '../../data/IndexPenData/IndexPenStudyData/IndexPenStudy1Data/8-13_5User_cr_(0.8,0.8)'
...
rD_max, rD_min = np.max(X_mmw_rD_model), np.min(X_mmw_rD_model)
rA_max, rA_min = np.max(X_mmw_rA_model), np.min(X_mmw_rA_model)

X_mmw_rD_model = (X_mmw_rD_model - rD_min) / (rD_max - rD_min)
X_mmw_rA_model = (X_mmw_rA_model - rA_min) / (rA_max - rA_min)

# upsampling rD map along axis (N, 120, 8, 16, 1) 16 * 4 -->64
X_mmw_rD_model = np.repeat(X_mmw_rD_model, 4, axis=3)

# stack through axis
X_mmw_rD_rA_combine_model = np.concatenate((X_mmw_rD_model, X_mmw_rA_model), axis=4)
del X_mmw_rD_model
del X_mmw_rA_model

X_mmw_rD_rA_combine_model_train = X_mmw_rD_rA_combine_model[train_indexes]
X_mmw_rD_rA_combine_model_test = X_mmw_rD_rA_combine_model[test_indexes]
#
...
model = make_complex_model_RA_RD_combine(class_num=31, learning_rate=1e-3, decay=1e-5,
                           ...
                           ra_kernel_size1=(3, 3), ra_kernel_size2=(3, 3), filter_num=16,
                           cv_reg=2e-5
                           )
...
# train  the model
history = model.fit([X_mmw_rD_rA_combine_model_train], Y_model_train,
                    validation_data=([X_mmw_rD_rA_combine_model_test], Y_model_test),
                    epochs=2000,
...
Y_model_pred1 = best_model.predict([X_mmw_rD_rA_combine_model_test])
